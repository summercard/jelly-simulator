bl_info={'name':'Q弹果冻 · 重力与固定切面','author':'Codex','version':(3,3,0),'blender':(4,2,0),'location':'3D视图 > N侧栏 > Q弹果冻','description':'固定切面、重力下垂、抓拉快甩、摇晃与硬软调节','category':'3D View'}
...
def contact_fields(rest,co,centre,radius,scene,normal):
    """Shared spatial field keeps facial meshes attached to the same tissue."""
    diff=co-centre;distance=np.linalg.norm(diff,axis=1)
    axial=diff@normal
    lateral2=np.maximum(0,distance*distance-axial*axial)
    cap=np.sqrt(np.maximum(0,radius*radius-lateral2))
    # Follow the hemisphere facing the tissue even after the sphere centre enters it.
    # Nearest-point radial projection would flip the dent outward at depth > radius.
    penetration=np.where(lateral2<radius*radius,np.maximum(0,axial+cap),0)
    # Continue the equator displacement through a soft shoulder instead of
    # abruptly zeroing it at the projected sphere cylinder.
    outside=np.maximum(np.sqrt(lateral2)-radius,0)
    shoulder=np.maximum(axial,0)*np.exp(-(outside/(radius*.30))**2)
    penetration=np.where(lateral2>=radius*radius,shoulder,penetration)
    target=-normal[None,:]*penetration[:,None]
    t=np.clip(rest[:,2]/.18,0,1);pin=t*t*(3-2*t)
    target*=pin[:,None]
    # Measure the lip in the CONTACT tangent plane, not distance from the
    # ball centre or the object's origin. Tissue piles up towards the ball.
    lateral=np.sqrt(lateral2)
    tangent=diff-axial[:,None]*normal[None,:]
    tangent/=np.maximum(lateral[:,None],1e-6)
    width=radius*(.24+.24*scene.jelly_bulge_spread)
    edge=np.clip((lateral-radius)/(radius*.38),0,1)
    edge=edge*edge*(3-2*edge)
    ring=np.exp(-((lateral-radius*1.25)/width)**2)*edge
    # Localise to the contacted surface; never raise the opposite side.
    front=np.exp(-(np.maximum(-axial-radius,0)/(radius*.65))**2)
    ring*=pin*front
    field=(normal[None,:]+.18*tangent)*ring[:,None]
    return target,field
...
def update_dents(st,dt):
    ...
    for obj,rest in st['entries']:
        ...
        else:target,field=contact_fields(rest,co,centre,radius,scene,st['pressure_normal'])
    ...
    if centre is not None and body_info:
        ...
        for _ in range(3):
            current=co+target+field*amplitude
            volume=mesh_volume(current,tri)
            derivative=(mesh_volume(current+field*.001,tri)-volume)/.001
    ...
    for obj,rest,co,target,field in fields:
        ...
        if centre is not None:
            # Project the moving tissue outside the sphere; the cap remains pinned.
            diff=co+offset-centre;distance=np.linalg.norm(diff,axis=1)
            normal=st['pressure_normal'];axial=diff@normal
            lateral2=np.maximum(0,distance*distance-axial*axial)
            cap=np.sqrt(np.maximum(0,radius*radius-lateral2))
            penetration=np.where(lateral2<radius*radius,np.maximum(0,axial+cap),0)
            t=np.clip(rest[:,2]/.18,0,1)
            offset-=normal[None,:]*penetration[:,None]*(t*t*(3-2*t))[:,None]
