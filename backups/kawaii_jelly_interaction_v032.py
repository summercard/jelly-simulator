bl_info={'name':'Q弹果冻 · 重力与固定切面','author':'Codex','version':(3,2,0),'blender':(4,2,0),'location':'3D视图 > N侧栏 > Q弹果冻','description':'固定切面、重力下垂、抓拉快甩、摇晃与硬软调节','category':'3D View'}
...
def contact_fields(rest,co,centre,radius,scene,normal):
    ...
    # No outward displacement in the ball contact patch; displaced tissue rises around it.
    edge=np.clip((distance-radius)/(radius*.55),0,1)
    edge=edge*edge*(3-2*edge)
    spread=radius*scene.jelly_bulge_spread
    ring=np.exp(-((distance-radius*.95)/spread)**2)*edge*pin
    field=outward*ring[:,None]
    return target,field
