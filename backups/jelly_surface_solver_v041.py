...
def init(st):
    ...
    st['surface_solver']={'render_edges':render_edges,'body':body,'rest':rest.copy(),'tri':tri,'edges':edges,'pins':pins,'followers':followers,'x':None,'velocity':np.zeros_like(rest),'contact_count':0}
    ...
    for d in data:
        ...
        for obj,rest in st['entries']:
            ...
            if obj==body:
                ...
                degree=np.bincount(edges.ravel(),minlength=len(rest)).clip(1)
                for _ in range(5):
                    for axis in range(3):
