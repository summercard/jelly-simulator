...
def init(st):
    ...
    pins=rest[:,2]<1e-7 if not st['anchor'].get('jelly_custom_soft') else np.zeros(len(rest),dtype=bool)
    ...
    for obj,r in st['entries']:
        ...
        ids=[];weights=[]
        for p in r:
            hits=tree.find_n(Vector(p),4);ids.append([h[1] for h in hits])
            w=1/np.maximum([h[2] for h in hits],1e-5)**2;weights.append(w/w.sum())
        followers[obj.as_pointer()]=(np.array(ids,dtype=np.int32),np.array(weights))
    st['surface_solver']={'body':body,'rest':rest.copy(),'tri':tri,'edges':edges,'pins':pins,'followers':followers,'x':None,'velocity':np.zeros_like(rest),'contact_count':0}
...
def edge_solve(d,base,h,k):
    ...
    for axis in range(3):
        ...
        x[:,axis]+=.25*weights*(neighbours/degree-offset[:,axis])
...
def step(scene,states,dt,api):
    ...
    for st in states:
        ...
        d['contact_count']=0;data.append(d)
    ...
    for substep in range(steps):
        ...
        for d in data:
            ...
            target,normals=volume_gradient(d['base'],d['tri']);d['target_volume']=target
        ...
        for iteration in range(scene.jelly_solver_iterations):
            for d in data:
                ...
                if contact_ids:
                    ...
                    d['influence']=.05+.95*np.exp(-distance2/(.3*scene.jelly_bulge_spread)**2)
                    current,_=volume_gradient(d['x'],d['tri'])
            ...
            if obj==body:
                ...
                for collider in colliders:
                    for i,p in collider.contacts(points,inward=inward):
                        if rest[i,2]>=1e-7 or d['root'].get('jelly_custom_soft'):points[i]=p
